...
void DialogYoutube::Diagnose() {
	...
	for (auto const& line : c->ass->Events) {
		...
		if (style) {
			...
			if (!fonts.count(font)) add(wxS("폰트 · ") + to_wx(style->font) + wxS(" → Roboto 계열 대체 · 모든 환경"));
			if (base && base->fontsize > 0) add(wxString::Format(wxS("크기 · 기준 %s %.1f 대비 %.1f%% · Android 크기 차이는 별도 확인"), to_wx(base->name), base->fontsize, style->fontsize / base->fontsize * 100));
		...
		for (auto group = boost::sregex_iterator(text.begin(), text.end(), groups); group != boost::sregex_iterator(); ++group) {
			...
			for (auto tag = boost::sregex_iterator(block.begin(), block.end(), tags); tag != boost::sregex_iterator(); ++tag) {
				...
				if (name == "fad" || name == "fade") add(wxS("페이드 · 그림자·외곽선 색이 #222222가 아니면 잔상이 남을 수 있음 · YouTube 전체"));
				if (name == "ytkt" || name == "ytshake" || name == "ytchroma" || name == "t") add(wxS("\\") + to_wx(name) + wxS(" · 이벤트 확장 및 파일 증가, 실기기 성능 확인 필요 · 모바일"));
				if (name == "ytruby" || name == "ytvert" || name == "ytsub" || name == "ytsup") add(wxS("\\") + to_wx(name) + wxS(" · 데스크톱 전용 또는 모바일 대체 표현 · 모바일"));
			}
		}
		boost::smatch match;
		if (boost::regex_search(text, match, position)) {
			try { double x = std::stod(match[1]), y = std::stod(match[2]);
				if (x < 0 || x > width || y < 0 || y > height) add(wxS("\\pos · PlayRes 밖 좌표, 실제 플레이어 표시 미검증 · 모든 환경"));
			} catch (std::exception const&) { add(wxS("\\pos · 좌표 숫자를 해석할 수 없음 · 모든 환경")); }
		}
		if (line.End <= line.Start) add(wxS("시간 · 종료가 시작보다 늦어야 함 · 모든 환경"));
		for (auto previous : active) if (line.Start < previous->End && previous->Start < line.End) {
			add(wxS("겹침 · 다른 행과 시간이 겹침 (실제 글자 충돌은 화면에서 확인) · 모든 환경")); break;
		}

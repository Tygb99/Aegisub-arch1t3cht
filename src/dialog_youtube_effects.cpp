...
wxPanel *DialogYoutube::MakeEffects(wxWindow *parent) {
	...
	for (auto label : {"위치 (X, Y)", "정렬 (1~9)", "글자 크기", "글자색", "외곽선 두께", "그림자 거리", "페이드 (등장·퇴장 ms)", "크기·색 변형 구간 추가", "가라오케 효과", "커서 앞 음절 시간 (ms)", "YouTube 전용 효과"}) property->Append(label);
	...
	auto first = number("X / 단일 값", w / 2, 100000);
	auto second = number("Y / 퇴장 ms", h * 9 / 10, 100000);
	auto start = number("변형 시작 ms", 0, 3600000);
	auto end = number("변형 종료 ms", 500, 3600000);
	grid->Add(new wxStaticText(panel, -1, "가속도"), 0, wxALIGN_CENTER_VERTICAL);
	auto accel = new wxSpinCtrlDouble(panel, -1, "1", wxDefaultPosition, wxSize(90, -1), wxSP_ARROW_KEYS, 0.01, 100, 1, 0.1);
	grid->Add(accel);
	grid->Add(new wxStaticText(panel, -1, "글자색 / 변형 끝색"), 0, wxALIGN_CENTER_VERTICAL);
	auto color = new wxColourPickerCtrl(panel, -1, *wxWHITE); grid->Add(color);
	layout->Add(grid, 0, wxALL, 10);
	auto kind = new wxChoice(panel, -1);
	for (auto label : {"Fade", "Glitch", "Cursor", "흔들기 (ytshake)", "색 분리 (ytchroma)", "루비 (ytruby)", "세로쓰기 (ytvert9)"}) kind->Append(label);
	kind->SetSelection(0);
	layout->Add(kind, 0, wxALL | wxEXPAND, 10);
	auto cursor = new wxTextCtrl(panel, -1, "_");
	cursor->SetToolTip("Cursor 효과의 문자. 쉼표·괄호·중괄호는 사용할 수 없습니다.");
	layout->Add(cursor, 0, wxALL | wxEXPAND, 10);
	auto effect_help = new wxStaticText(panel, -1, "Fade: 음절을 부드럽게 강조합니다. 예: {\\ytktFade}{\\k25}한{\\k25}글 · 파일이 커질 수 있습니다.");
	...
	kind->Bind(wxEVT_CHOICE, [=](wxCommandEvent&) {
		const wxString examples[] = {
			"Fade: 예 {\\ytktFade}{\\k25}한{\\k25}글 · 음절 강조, 파일 증가 가능",
			"Glitch: 예 {\\ytktGlitch}{\\k25}한{\\k25}글 · 임의 글자로 전환, 왼쪽 정렬 권장",
			"Cursor: 예 {\\ytkt(Cursor,_)}{\\k25}한{\\k25}글 · 현재 음절 뒤의 커서 문자",
			"흔들기: 예 {\\ytshake(20)} · X / 단일 값은 반경 px, 이벤트 수 증가",
			"색 분리: 예 {\\ytchroma} · 기본 20px, 등장·퇴장 270ms, 이벤트 수 증가",
			"루비: 예 {\\ytruby}[漢/かん] · 데스크톱 위첨자, 모바일은 괄호로 대체",
			"세로쓰기: 예 {\\ytvert9}세로 글 · 오른쪽부터 세로 열, 데스크톱 전용"
		};
		effect_help->SetLabel(examples[kind->GetSelection()]); effect_help->Wrap(680); panel->Layout();
	});
	auto explanation = new wxStaticText(panel, -1, "선택한 속성만 변경합니다. 기존 태그 편집·스타일 편집·가라오케 시간 도구도 그대로 사용하세요.\n위치는 현재 PlayRes 범위 안에서 설정합니다. 화면 밖 좌표의 실제 플레이어 지원은 미검증입니다.\n변형은 행 시작 기준 ms이며 구간을 반복 추가할 수 있습니다.\n음절 시간은 텍스트 커서 앞에 삽입됩니다. ASS \\k 단위는 10ms입니다.");
	explanation->Wrap(680); layout->Add(explanation, 0, wxALL, 10);
	auto apply = new wxButton(panel, -1, "변경 내용 확인 후 선택 행에 적용…");
	...
	apply->Bind(wxEVT_BUTTON, [=](wxCommandEvent&) {
		...
		if (property->GetSelection() == 4 || property->GetSelection() == 5) {
			...
			auto label = property->GetSelection() == 4 ? "외곽선" : "그림자";
			if (wxMessageBox(wxString::Format("선택 행 전용 스타일을 복제하고 %s 값을 %d로 설정합니다.\nYouTube는 스타일의 외곽선·그림자를 사용하며 두께는 동일하게 재현되지 않습니다.\n실행 취소할 수 있습니다.", label, value), "스타일 변경 확인", wxOK | wxCANCEL, this) != wxOK) return;
			...
			c->ass->Commit("YouTube 선택 행 스타일", AssFile::COMMIT_STYLES | AssFile::COMMIT_DIAG_META | AssFile::COMMIT_SCRIPTINFO);
			return;
		}
		switch (property->GetSelection()) {
			case 0:
				if (value > w || second->GetValue() > h) { wxMessageBox("현재 PlayRes 범위 안의 좌표를 입력하세요.", "위치", wxOK, this); return; }
			...
			case 7:
				if (start->GetValue() >= end->GetValue() || end->GetValue() > line->End - line->Start || value < 1) {
					wxMessageBox("변형 시작 < 종료 ≤ 행 길이, 크기 > 0이어야 합니다.", "변형 구간", wxOK, this); return;
				}
		...
		if (wxMessageBox("행 " + wxString::Format("%d", line->Row + 1) + "에 적용: {" + tag + "}\n실행 취소할 수 있습니다.", "변경 확인", wxOK | wxCANCEL, this) != wxOK) return;
		if (property->GetSelection() == 9) {
			...
			if (prefix.rfind('{') != std::string::npos && (prefix.rfind('}') == std::string::npos || prefix.rfind('{') > prefix.rfind('}'))) {
				wxMessageBox("태그 바깥의 음절 앞에 텍스트 커서를 놓으세요.", "음절 시간", wxOK, this); return;
			}
		...
		c->ass->Commit("YouTube 선택 행 효과", AssFile::COMMIT_DIAG_TEXT, -1, line);
	});
	return panel;
}

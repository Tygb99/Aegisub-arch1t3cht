...
wxPanel *DialogYoutube::MakeOptions(wxWindow *parent) {
	...
	layout->Add(new wxStaticText(panel, -1, "프로젝트별 설정은 ASS에 저장됩니다. 미리보기와 내보내기에 같은 설정을 사용합니다."), 0, wxALL, 12);
	...
	glow = new wxCheckBox(panel, -1, "외곽 광선 (Glow)");
	bevel = new wxCheckBox(panel, -1, "입체 테두리 (Bevel)");
	soft = new wxCheckBox(panel, -1, "부드러운 그림자");
	hard = new wxCheckBox(panel, -1, "단단한 그림자");
	...
	karaoke = new wxCheckBox(panel, -1, "현재 가라오케 음절 강조");
	layout->Add(karaoke, 0, wxALL, 12);
	highlight = new wxTextCtrl(panel, -1, "#FFFF00");
	layout->Add(new wxStaticText(panel, -1, "현재 음절 글자색 (#RRGGBB)"), 0, wxLEFT, 12);
	layout->Add(highlight, 0, wxALL | wxEXPAND, 12);
	auto profile = new wxBoxSizer(wxHORIZONTAL);
	profile->Add(new wxStaticText(panel, -1, "출력 배율"), 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 6);
	...
	profile->Add(new wxStaticText(panel, -1, "명시적 위치 이동 X / Y"), 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 6);
	...
	mobile = new wxCheckBox(panel, -1, "모바일 단순화 출력: 효과·위치 제거, 흰 글자·검은 배경·하단 중앙");
	layout->Add(mobile, 0, wxLEFT | wxRIGHT, 12);
	auto save = new wxButton(panel, -1, "스타일·출력 설정 적용 (실행 취소 가능)");
...
void DialogYoutube::SaveStyle() {
	...
	if (!color.IsOk()) { status->SetLabel("올바른 강조색을 입력하세요 (#RRGGBB)."); return; }
	auto doc = Options(c->ass.get());
	auto node = FindStyle(doc, style->GetStringSelection());
	...
	c->ass->Commit("YouTube 스타일 변환 설정", AssFile::COMMIT_SCRIPTINFO);
}

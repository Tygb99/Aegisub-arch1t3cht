...
: wxDialog(context->parent, -1, "YouTube 자막 · 변환과 비교", wxDefaultPosition, wxSize(760, 670), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
, c(context), debounce(this), player_poll(this) {
	...
	preview = new wxCheckBox(this, -1, "영상 화면에서 YouTube 변환 결과 보기 (시각적 근사치)");
	layout->Add(preview, 0, wxALL, 12);
	layout->Add(new wxStaticText(this, -1, "체크를 끄면 같은 시각의 편집 원본을 봅니다. 재생·탐색은 기존 영상 도구를 사용하세요."), 0, wxLEFT | wxRIGHT, 12);
	status = new wxStaticText(this, -1, "변환 준비 중");
	layout->Add(status, 0, wxALL | wxEXPAND, 12);
	auto book = new wxNotebook(this, -1);
	book->AddPage(MakeOptions(book), "변환 설정");
	book->AddPage(MakeEffects(book), "선택 행 효과");
	issues = new wxListCtrl(book, -1, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL);
	issues->InsertColumn(0, "행 / 시간", wxLIST_FORMAT_LEFT, 125);
	issues->InsertColumn(1, "태그 · 원인 · 보기 환경 (두 번 클릭하여 이동)", wxLIST_FORMAT_LEFT, 580);
	book->AddPage(issues, "호환성 진단");
	...
	url->SetHint("YouTube 영상 URL");
	language = new wxTextCtrl(this, -1, "en", wxDefaultPosition, wxSize(55, -1));
	language->SetToolTip("대체할 기존 수동 자막의 언어 코드 (en, ko 등). 자동 생성·자동 번역 트랙은 제외됩니다.");
	player_button = new wxButton(this, -1, "실제 플레이어 시작");
	auto stop_player = new wxButton(this, -1, "플레이어 종료");
	...
	player_status = new wxStaticText(this, -1, "전용 Chrome에서 기존 자막 트랙을 켜세요. 실제 모바일 기기 검증은 별도로 필요합니다.");
	layout->Add(player_status, 0, wxLEFT | wxRIGHT | wxEXPAND, 12);
	auto buttons = new wxBoxSizer(wxHORIZONTAL);
	auto refresh = new wxButton(this, -1, "다시 변환");
	auto cancel = new wxButton(this, -1, "변환 취소");
	auto import = new wxButton(this, -1, "YTT → 편집용 ASS");
	export_button = new wxButton(this, -1, "내보내기 묶음…");
	auto edit = new wxButton(this, -1, "영상·자막 편집");
	edit->SetToolTip("창을 숨기고 편집합니다. 자동 변환은 계속됩니다. 파일 메뉴에서 다시 여세요.");
	...
	cancel->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
		...
		Invalidate(); pending = false; debounce.Stop(); status->SetLabel("취소됨 · 최신 결과가 없습니다.");
	});
	...
	Bind(wxEVT_TIMER, [this](wxTimerEvent&) {
		...
		else if (message.Contains("오류") || message.Contains("실패")) player_status->SetLabel("플레이어 세션: " + message);
	}, player_poll.GetId());
...
void DialogYoutube::Invalidate() {
	...
	status->SetLabel("편집됨 · 변환 대기 중 (이전 결과 사용 안 함)");
	...
	if (player_pid) player_status->SetLabel("편집됨 · 최신 결과 변환 대기 중. 플레이어에는 이전 자막이 남아 있을 수 있습니다.");
...
void DialogYoutube::StartConversion() {
	...
	Diagnose();
	if (!wxFileExists(Helper())) { status->SetLabel("변환 엔진이 없습니다. YouTube 보조 프로그램을 포함한 패키지를 사용하세요."); return; }
	...
	try {
		...
		if (!options.IsOpened() || !options.Write(xml) || !options.Close())
			throw std::runtime_error("변환 설정 파일 저장 실패");
		...
		if (!pid) { delete process; process = nullptr; status->SetLabel("변환 프로세스를 시작하지 못했습니다."); return; }
		status->SetLabel("변환 중… 취소할 수 있습니다.");
...
void DialogYoutube::Finish(wxProcessEvent &event) {
	if (event.GetPid() == import_pid) {
		...
		status->SetLabel(event.GetExitCode() == 0 ? "편집용 ASS 생성됨: " + wxFileName(import_output).GetFullName() : "가져오기 실패 또는 취소: " + error);
		return;
	}
	if (event.GetPid() == player_pid) {
		...
		player_status->SetLabel(error.empty() ? "플레이어 세션 종료됨" : error); return;
	}
	...
	if (event.GetExitCode() != 0) { status->SetLabel("변환 실패: " + error); return; }
	try {
		...
		auto summary = wxString::Format("최신 변환 완료 · 입력 후 %lld ms · %s bytes · 시각적 근사치", (long long)elapsed,
		...
		summary += wxString::Format("\n입력 %lld행 → 확장 %lld개 · 초당 %s개 · %s",
			...
			(bool)output_profile["Mobile"] ? "모바일 단순화 (실기기 미검증)" : "원본 스타일 출력");
		if (!wxCopyFile(running_dir + "/result.ytt", root + "/current.ytt.tmp", true) ||
			!wxRenameFile(root + "/current.ytt.tmp", root + "/current.ytt", true)) throw std::runtime_error("YTT 결과 파일 적용 실패");
		...
		if (player_pid) player_status->SetLabel("최신 결과 준비됨 · CC를 껐다 켜세요. 현재 YTT 응답 대체는 아직 확인되지 않았습니다.");
		export_button->Enable(); player_button->Enable();
		UpdateView();
	} catch (std::exception const& error) {
		visual.reset(); ready_revision = 0; export_button->Disable(); player_button->Disable();
		if (wxFileExists(root + "/current.ytt")) wxRemoveFile(root + "/current.ytt");
		UpdateView(); status->SetLabel("미리보기 로드 실패: " + to_wx(error.what()));
	}
}
